...
@implementation FunFaceViewController
...
- (void) opencvFaceDetect:(UIImage *)overlayImage  {
	...
	if(imageView.image) {
		...
		for(int i = 0; i < faces->total; i++) {
            ...
            //draw a bound rectangle 
            CGContextStrokeRect(contextRef, face_rect);
            //extract the face
            CGImageRef aFaceImageRef = CGImageCreateWithImageInRect(imageRef, face_rect);
            //upload the face
            [self uploadFace:aFaceImageRef];
            
            CFRelease(aFaceImageRef);
...
- (void) uploadFace:(CGImageRef) aFaceImgRef{
...
- (IBAction)loadImage:(id)sender {
	if(!actionSheetAction) {
		UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@""
														...
														otherButtonTitles:@"Do FunFace with Photo Library", @"Take Photo and Do FunFace", @"Do FunFace With My Photo", nil];
